...
#define MIN_SIZE 3.0f
	...
	unsigned int loadsnowTexture(char const * path) {
	...
	Snow::Snow()
	{
		...
		mUpdateShader = new Shader("../Particle/shader/Update.vs", "../Particle/shader/Update.fs", 
						"../Particle/shader/Update.gs", varyings, 5);
		//设置TransformFeedback缓存能够记录的顶点的数据类型

		mRenderShader = new Shader("../Particle/shader/Render.vs", 
							"../Particle/shader/Render.fs");
		//设置随机纹理
		InitRandomTexture(512);
		mSparkTexture = loadsnowTexture("../Particle/Textures/snowstorm.bmp");
		mRenderShader->use();
